...
def plot_gaussian_conditionals(ax=None, color_map=None):
    ...
    x_smooth = np.linspace(petal_length.min() - 0.5,
    ...
    for i, species in enumerate(iris.target_names):
        ...
        counts, bins, _ = ax.hist(species_data, bins=n_bins, density=True,
                                  color=color_map[f'c{i+1}'], alpha=0.3,
                                  label=f'{species} (data)', edgecolor='none')

        # Fit and plot Gaussian
        mu = np.mean(species_data)
        sigma = np.std(species_data)
        gaussian = norm.pdf(x_smooth, mu, sigma)
        ax.plot(x_smooth, gaussian, '--', color=color_map[f'c{i+1}'],
                label=f'{species} (Gaussian)', linewidth=2, alpha=0.8)

    # Customize plot appearance
    ax.set_title('Iris Petal Length Distribution with Gaussian Fits',
                 fontsize=12, pad=15)
    ...
    ax.legend(frameon=True, framealpha=0.9, loc='upper right', fontsize=9,
              ncol=2)  # Two columns for cleaner legend
    ...
    max_density = max(norm.pdf(mu, mu, sigma) for mu, sigma in
