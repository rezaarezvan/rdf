...
def plot_convex_nonconvex_sets(ax=None, color_map=None):
    ...
    fig = ax.figure
    ax.remove()
    gs = fig.add_gridspec(1, 2, height_ratios=[
                          1], width_ratios=[1, 1], wspace=0.3)
    ...
    ellipse = Ellipse((0, 0), 3, 1.8, angle=15,
                      facecolor=color_map["c8"],
                      edgecolor=color_map["c8"], linewidth=2)
    ...
    ax1.plot([x1, x2], [y1, y2], color="black",
             linewidth=2, zorder=4, label='Line segment')
    ...
    patch = PathPatch(path, facecolor=color_map["c8"],
                      edgecolor=color_map["c8"], linewidth=2)
    ax2.add_patch(patch)
    ...
    ax2.plot([x1_nc, x2_nc], [y1_nc, y2_nc], color="black",
             linewidth=3, zorder=4, linestyle='--', alpha=0.8)
...
def plot_unit_sphere_3d(ax=None, color_map=None):
    ...
    # Plot surface
    ax.plot_surface(x, y, z, alpha=0.6, color=color_map["c8"],
                    linewidth=0.5, edgecolors=color_map["c8"])
...
def plot_circle_boundary(ax=None, color_map=None):
    ...
    ax.plot(x_circle, y_circle, color=color_map["c8"],
            linewidth=3, label='Unit Circle Boundary')
...
def plot_overlapping_sets(ax, color_map):
    ellipse1 = Ellipse((0, 0), 1.4, 0.8, angle=0,
                       facecolor=color_map["c8"], alpha=0.6,
                       edgecolor=color_map["c8"], linewidth=2)
    ellipse2 = Ellipse((0.3, 0), 1.4, 0.8, angle=5,
                       facecolor=color_map["c8"], alpha=0.4,
                       edgecolor=color_map["c8"], linewidth=2, linestyle='--')
...
def plot_disconnected_sets(ax, color_map):
    ...
    ellipse1 = Ellipse((-0.8, 0), 0.8, 0.6, angle=0,
                       facecolor=color_map["c8"], alpha=0.6,
                       edgecolor=color_map["c8"], linewidth=2)
    ellipse2 = Ellipse((0.8, 0), 0.8, 0.6, angle=0,
                       facecolor=color_map["c8"], alpha=0.6,
                       edgecolor=color_map["c8"], linewidth=2)
...
def plot_minimal_convex_hull(ax, color_map):
    ...
    c_patch = PathPatch(c_path, facecolor=color_map["c8"], alpha=0.6,
                        edgecolor=color_map["c8"], linewidth=2)
    ax.add_patch(c_patch)

    # Fill in the "mouth" to show convex hull
    # Add the straight line segment that closes the C
    mouth_vertices = [(1.0 * np.cos(-2.2), 1.0 * np.sin(-2.2)),
                      (1.0 * np.cos(2.2), 1.0 * np.sin(2.2))]

    ax.scatter([v[0] for v in mouth_vertices], [v[1] for v in mouth_vertices],
               s=30, color='black', zorder=5)

    # Draw the closing line prominently
    ax.plot([mouth_vertices[0][0], mouth_vertices[1][0]],
            [mouth_vertices[0][1], mouth_vertices[1][1]],
            color='black', linewidth=2)

    ax.text(0, -1.4, 'Minimal Convex Set', ha='center',
...
def plot_larger_convex_sets(ax, color_map):
    ...
    large_ellipse1 = Ellipse((0, 0), 3.2, 2.8, angle=0,
                             facecolor='none', edgecolor='gray',
                             linewidth=2, linestyle='--', alpha=0.7)
    ax.add_patch(large_ellipse1)

    large_ellipse2 = Ellipse((0, 0), 2.8, 2.4, angle=15,
                             facecolor='none', edgecolor='gray',
                             linewidth=2, linestyle='--', alpha=0.7)
    ax.add_patch(large_ellipse2)

    ax.text(0, -2, 'Larger Convex Sets', ha='center',
            fontsize=12, fontweight='bold')
...
def plot_convex_combinations(ax, color_map):
    ...
    for start, end in arrow_pairs:
        ax.scatter(start[0], start[1], s=20,
                   color='black', alpha=0.7, zorder=4)
        ax.annotate('', xy=end, xytext=start,
                    arrowprops=dict(arrowstyle='->', color='black',
                                    lw=1.5, alpha=0.7))
        ax.scatter(end[0], end[1], s=20,
                   color='black', alpha=0.7, zorder=4)

    ax.text(0, -1.4, 'All Convex Combinations',
...
def plot_triangle_with_interior_point(ax, color_map):
    ...
    triangle = Polygon([v1, v2, v3], facecolor=color_map["c8"], alpha=0.3,
                       edgecolor=color_map["c8"], linewidth=2)
    ...
    # Add labels
    ax.text(v1[0]-0.15, v1[1]-0.15, '$v_1$',
            fontsize=12, ha='center', fontweight='bold')
    ax.text(v2[0]+0.15, v2[1]-0.15, '$v_2$',
            fontsize=12, ha='center', fontweight='bold')
    ax.text(v3[0], v3[1]+0.15, '$v_3$', fontsize=12,
            ha='center', fontweight='bold')
    ax.text(v4[0]+0.2, v4[1], '$v_4$', fontsize=12,
            ha='center', fontweight='bold', color='red')

    # Show convex combination with dashed lines
    ax.plot([v1[0], v4[0]], [v1[1], v4[1]], 'gray',
            linestyle='--', alpha=0.6, linewidth=1)
    ax.plot([v2[0], v4[0]], [v2[1], v4[1]], 'gray',
            linestyle='--', alpha=0.6, linewidth=1)
    ax.plot([v3[0], v4[0]], [v3[1], v4[1]], 'gray',
            linestyle='--', alpha=0.6, linewidth=1)
...
def plot_triangle_extreme_only(ax, color_map):
    ...
    ax.plot([v1[0], v2[0]], [v1[1], v2[1]],
            color_map["c8"], linewidth=3, alpha=0.8)
    ax.plot([v2[0], v3[0]], [v2[1], v3[1]],
            color_map["c8"], linewidth=3, alpha=0.8)
    ax.plot([v3[0], v1[0]], [v3[1], v1[1]],
            color_map["c8"], linewidth=3, alpha=0.8)
...
def plot_halfspace(ax=None, color_map=None):
    ...
    ax.plot(line1_x, line1_y, color=color_map["c8"],
            linewidth=2, label='$x_1 + x_2 \\leq 3$')

    infeasible1 = X + Y > 3
    ax.contourf(X, Y, infeasible1.astype(int), levels=[0.5, 1.5],
                colors=color_map["c8"], alpha=0.15)
    ...
    legend_elements = [
        Patch(facecolor=color_map["c8"], alpha=0.15,
...
def plot_convex_cone(ax, color_map):
    ...
    cone_region = Polygon(vertices, facecolor=color_map["c8"], alpha=0.3,
                          edgecolor=color_map["c8"], linewidth=2)
    ...
    # Draw scaling arrows
    ax.plot([0, scaled_point[0]], [0, scaled_point[1]],
            'red', linewidth=1, alpha=0.7, linestyle='--')
    ax.plot([0, scaled_point2[0]], [0, scaled_point2[1]],
            'red', linewidth=1, alpha=0.7, linestyle='--')

    # Labels
    ax.text(example_point[0]+0.1, example_point[1] +
            0.1, '$x$', fontsize=11, ha='left')
    ax.text(scaled_point[0]-0.2, scaled_point[1],
            '$0.6x$', fontsize=10, ha='right', color='red')
    ax.text(scaled_point2[0]+0.1, scaled_point2[1],
            '$1.4x$', fontsize=10, ha='left', color='red')
...
def plot_nonconvex_cones(ax, color_map):
    ...
    from matplotlib.patches import Polygon
    cone1 = Polygon(vertices1, facecolor=color_map["c8"], alpha=0.4,
                    edgecolor=color_map["c8"], linewidth=2)
    ax.add_patch(cone1)
    ...
    cone2 = Polygon(vertices2, facecolor=color_map["c8"], alpha=0.4,
                    edgecolor=color_map["c8"], linewidth=2)
    ax.add_patch(cone2)

    # Draw boundary rays
    ax.plot([0, r_max * np.cos(angle1_start)],
            [0, r_max * np.sin(angle1_start)], 'black', linewidth=2)
    ax.plot([0, r_max * np.cos(angle1_end)],
            [0, r_max * np.sin(angle1_end)], 'black', linewidth=2)
    ax.plot([0, r_max * np.cos(angle2_start)],
            [0, r_max * np.sin(angle2_start)], 'black', linewidth=2)
    ax.plot([0, r_max * np.cos(angle2_end)],
            [0, r_max * np.sin(angle2_end)], 'black', linewidth=2)

    # Show line segment between cones that goes outside
    point1 = np.array([1.5 * np.cos(np.pi/4), 1.5 *
                      np.sin(np.pi/4)])  # In cone 1
    point2 = np.array([1.5 * np.cos(-np.pi/4), 1.5 *
                      np.sin(-np.pi/4)])  # In cone 2

    ax.plot([point1[0], point2[0]], [point1[1], point2[1]], 'black',
            linewidth=2, linestyle='--', alpha=0.8)
