...
def plot_example_linear_program(ax=None, color_map=None):
    """
    Create a clean visualization of the linear program,

    Consider the LP,
    min -x_2
    subject to x_1 - 2x_2 <= 2,
               x_1 - x_2 <= 3,
               x_2 <= 3,
               x_1, x_2 >= 0
    ...
    # Define the constraint lines
    x_vals = np.linspace(x_min, x_max, 400)
    y1 = (x_vals - 2) / 2  # From x1 - 2x2 <= 2  => x2 >= (x1 - 2)/2
    y2 = x_vals - 3  # From x1 - x2 <= 3  => x2 >= x1 - 3
    y3 = np.full_like(x_vals, 3)  # x2 <= 3
    y4 = np.zeros_like(x_vals)  # x2 >= 0
    x4 = np.zeros_like(x_vals)  # x1 >= 0
    y5 = np.full_like(x_vals, y_max)  # x2 <= y_max
    x5 = np.full_like(x_vals, x_max)  # x1 <= x_max

    # Plot the constraint lines
    ax.plot(
        x_vals, y1, color="black", linewidth=2, label=r"$x_1 - 2x_2 \leq 2$", zorder=4
    )
    ax.plot(
        x_vals, y2, color="black", linewidth=2, label=r"$x_1 - x_2 \leq 3$", zorder=4
    )
    ax.plot(x_vals, y3, color="black", linewidth=2, label=r"$x_2 \leq 3$", zorder=4)
    ax.plot(x_vals, y4, color="black", linewidth=2, label=r"$x_2 \geq 0$", zorder=4)
    ax.plot(x4, x_vals, color="black", linewidth=2, label=r"$x_1 \geq 0$", zorder=4)
    ax.plot(x_vals, y5, color="black", linewidth=1, linestyle="dashed", zorder=2)
    ax.plot(x5, x_vals, color="black", linewidth=1, linestyle="dashed", zorder=2)

    # Fill the feasible region
    ax.fill_between(
        x_vals,
        np.maximum(np.maximum(y1, y2), y4),
        np.minimum(y3, y5),
        where=(x_vals >= 0) & (y1 <= y_max) & (y2 <= y_max),
        color=color_map["c8"],
        alpha=0.5,
...
if __name__ == "__main__":
    ...
    # svg_content = plotter.create_themed_plot(
    #     save_name="example_polyhedron_regions2", plot_func=plot_example_polyhedron_regions2
    # )

    svg_content = plotter.create_themed_plot(
        save_name="example_linear_program", plot_func=plot_example_linear_program
    )
