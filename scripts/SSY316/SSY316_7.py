...
def plot_multi_class_regression_linear_and_quadratic(ax=None, color_map=None):
    ...
    y = np.concatenate(
        [np.zeros(len(x0)), np.ones(len(x1)), np.full(len(x2), 2)])
...
def plot_multi_class_lda(ax=None, color_map=None):
    ...
    for i, (mean, cov, color) in enumerate(zip(means, covs, contour_colors)):
        ...
        density = rv.pdf(pos)
        # Plot contours
        ax[1].contour(xx, yy, density, levels=8,
                      colors=color, linewidths=1, alpha=0.6)
...
def plot_monte_carlo_inference(ax=None, color_map=None):
    ...
    p_z = (
        0.9 * multivariate_normal.pdf(x, mean=-1.5, cov=0.4) +
...
if __name__ == "__main__":
    ...
    # svg_content = plotter.create_themed_plot(
    #     save_name="monte_carlo_inference", plot_func=plot_monte_carlo_inference
    # )

    svg_content = plotter.create_themed_plot(
        save_name="monte_carlo_inference_importance_sampling",
