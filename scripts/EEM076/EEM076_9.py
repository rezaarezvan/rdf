...
def plot_mutual_inductance(ax, color_map):
    ...
    _coil(ax, 0, coil)
    _coil(ax, 2.6, coil)
    ax.text(-3.2, 0.45, "Coil 1", color=ink, ha="left")
    ax.text(-3.2, 3.05, "Coil 2", color=ink, ha="left")
    ax.text(1.15, 0, "$N_1$", color=ink, va="center")
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    ax.set(xlim=(-3.4, 3.6), ylim=(-2.6, 4.2))
